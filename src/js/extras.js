    ...
    for (const fmt of formats) {
      ...
      try {
        if (fmt === 'png') {
          ...
          const pdfBlob = await html2pdf().set({ margin: [8, 8], filename: 'cv-genie-' + t + '.pdf', image: { type: 'jpeg', quality: 0.95 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } }).from(document.getElementById('cvPreview')).outputPdf('blob');
          zip.file('cv-genie-' + t + '.pdf', pdfBlob);
          successCount++;
        } else if (fmt === 'server-pdf') {
          const preview = document.getElementById('cvPreview');
          const cs = getComputedStyle(document.documentElement);
          const r = await fetch(PDF_SERVER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ html: preview.outerHTML, color: cs.getPropertyValue('--cv-color').trim() || '#2563eb', fontHeading: cs.getPropertyValue('--cv-font-heading').trim() || 'Georgia, serif', fontBody: cs.getPropertyValue('--cv-font-body').trim() || 'Georgia, serif' }) });
          if (r.ok) { const b = await r.blob(); if (b.size >= 1000) { zip.file('cv-genie-' + t + '-hq.pdf', b); successCount++; } }
        } else if (fmt === 'docx') {
          const data = { name: document.getElementById('fullName')?.value || 'Your Name', title: document.getElementById('title')?.value || '', contact: [document.getElementById('email')?.value, document.getElementById('phone')?.value, document.getElementById('linkedin')?.value, document.getElementById('location')?.value].filter(Boolean).join(' | '), summary: document.getElementById('summary')?.value || '', skills: document.getElementById('skills')?.value || '', experiences: getExperiences(), education: getEducation() };
          const r = await fetch(DOCX_SERVER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
          if (r.ok) { const b = await r.blob(); if (b.size >= 1000) { zip.file('cv-genie-' + t + '.docx', b); successCount++; } }
        } else if (fmt === 'rtf') {
          const name = document.getElementById('fullName')?.value || 'Your Name', title = document.getElementById('title')?.value || '';
          ...
          const summary = document.getElementById('summary')?.value || '', skills = document.getElementById('skills')?.value || '', exp = getExperiences(), edu = getEducation();
          ...
          if (exp.length) { rtfStr += '\pard\sb200\sa60{\f1\fs22\b\caps EXPERIENCE}\par\n'; exp.forEach(x => { rtfStr += '\pard\sb60\sa0{\f1\fs22\b ' + esc(x.title || '') + '}\par\n\pard\sb0\sa0{\f1\fs20\cf2 ' + esc(x.company || '') + ' — ' + esc(x.dates || '') + '}\par\n'; if (x.description) rtfStr += '\pard\li360\sb0\sa80{\f1\fs20 ' + esc(x.description) + '}\par\n'; }); }
          if (edu.length) { rtfStr += '\pard\sb200\sa60{\f1\fs22\b\caps EDUCATION}\par\n'; edu.forEach(x => { rtfStr += '\pard\sb40\sa0{\f1\fs22\b ' + esc(x.degree || '') + '}\par\n\pard\sb0\sa60{\f1\fs20\cf2 ' + esc(x.institution || '') + ' — ' + esc(x.year || '') + '}\par\n'; }); }
      ...
      } catch (err) { console.warn('Failed to export template ' + t + ' as ' + fmt + ':', err); }
    }
  }

  changeTemplate(orig);
  if (successCount === 0) { s.textContent = 'Export failed. Check browser console.'; setTimeout(() => s.style.display = 'none', 5000); return; }
...
function openPhotoEditor() {
  if (!photoData) { document.getElementById('photoInput').click(); return; }
  ...
  cropperInstance = new Cropper(document.getElementById('cropperImage'), { aspectRatio: 1, viewMode: 1, dragMode: 'move', autoCropArea: 0.9 });
}
function closePhotoEditor() {
  document.getElementById('photoCropperModal').style.display = 'none';
  if (cropperInstance) { cropperInstance.destroy(); cropperInstance = null; }
}
function rotatePhoto(deg) { if (cropperInstance) cropperInstance.rotate(deg); }
function applyCrop() {
...
function openTemplateGallery() {
  ...
  document.getElementById('templateGalleryGrid').innerHTML = TEMPLATES.map(t => `<div class="template-card${t === currentTemplate ? ' active' : ''}" onclick="selectTemplateFromGallery('${t}')"><div class="thumb">${TEMPLATE_META[t].icon}<span class="active-badge">Active</span></div><div class="card-info"><strong>${TEMPLATE_META[t].name}</strong><small>${TEMPLATE_META[t].desc}</small></div></div>`).join('');
}
function closeTemplateGallery() { document.getElementById('templateGalleryModal').style.display = 'none'; }
function selectTemplateFromGallery(t) { changeTemplate(t); openTemplateGallery(); setTimeout(closeTemplateGallery, 400); }
function updateTemplateGallerySelection() { if (document.getElementById('templateGalleryModal').style.display === 'flex') document.getElementById('templateGalleryGrid').innerHTML = TEMPLATES.map(t => `<div class="template-card${t === currentTemplate ? ' active' : ''}" onclick="selectTemplateFromGallery('${t}')"><div class="thumb">${TEMPLATE_META[t].icon}<span class="active-badge">Active</span></div><div class="card-info"><strong>${TEMPLATE_META[t].name}</strong><small>${TEMPLATE_META[t].desc}</small></div></div>`).join(''); }
function saveVersion() {
  const s = { timestamp: new Date().toLocaleString(), state: getState(), preview: (document.getElementById('fullName')?.value || 'Untitled') + ' - ' + currentTemplate };
...
function loadVersion(i) { if (!versionSlots[i]) return; restoreState(versionSlots[i].state); }
function deleteVersion(i) { versionSlots.splice(i, 1); try { localStorage.setItem('cvGenieVersions', JSON.stringify(versionSlots)); } catch (e) { } renderVersionSlots(); }
function renderVersionSlots() {
  const c = document.getElementById('versionSlots');
  if (!c) return;
  c.innerHTML = versionSlots.length === 0 ? '<p style="text-align:center;color:var(--text-secondary);font-size:0.65rem">No versions yet.</p>' : versionSlots.map((s, i) => `<div class="version-slot"><div class="slot-date">${s.timestamp}</div><div style="font-size:0.55rem;margin:0.12rem 0">${s.preview || ''}</div><div class="slot-actions"><button class="btn btn-sm btn-primary" onclick="loadVersion(${i})">Load</button><button class="btn btn-sm" onclick="deleteVersion(${i})" style="background:#fee2e2;color:#dc2626">×</button></div></div>`).join('');
}
...
function copyShareLink() { const l = document.getElementById('shareLinkBox')?.dataset?.fullLink; if (l) navigator.clipboard.writeText(l).then(() => showToast('Copied!')); }
function renderCustomKWChips() {
  const c = document.getElementById('customKeywordChips');
  if (!c) return;
  c.innerHTML = customKWList.map((k, i) => `<span style="display:inline-flex;align-items:center;gap:0.2rem;background:var(--primary);color:white;border-radius:20px;padding:0.15rem 0.5rem;font-size:0.6rem;font-weight:600;">${k}<button onclick="removeCustomKW(${i})" style="background:none;border:none;color:white;cursor:pointer;font-size:0.7rem;padding:0;line-height:1;opacity:0.8;">×</button></span>`).join('');
}
...
function removeCustomKW(i) { customKWList.splice(i, 1); renderCustomKWChips(); runATSAnalysis(); }
function addKWFromCloud(kw) { const k = kw.trim().toLowerCase(); if (k && !customKWList.includes(k)) { customKWList.push(k); renderCustomKWChips(); runATSAnalysis(); } }
function handleKWKey(e) { if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addCustomKeyword(); } }
async function importLinkedInPDF(e) {
  ...
  try {
    ...
    let t = '';
    for (let i = 1; i <= pdf.numPages; i++) { const pg = await pdf.getPage(i); const c = await pg.getTextContent(); t += c.items.map(it => it.str).join(' ') + '\n'; }
  ...
  } catch (e) { showToast('Error reading PDF'); document.getElementById('linkedInProgress').style.display = 'none'; }
}
function applyLinkedInData() {
  ...
  (d.experiences || []).forEach(e => { addExpSilent(); const is = document.querySelectorAll('#experienceList .dynamic-item'); const l = is[is.length - 1]; if (l) { l.querySelectorAll('input')[0].value = e.title || ''; l.querySelectorAll('input')[1].value = e.company || ''; l.querySelectorAll('input')[2].value = e.dates || ''; l.querySelector('textarea').value = e.description || ''; } });
  if (!d.experiences || !d.experiences.length) addExpSilent();
  document.getElementById('educationList').innerHTML = '';
  (d.education || []).forEach(e => { addEduSilent(); const is = document.querySelectorAll('#educationList .dynamic-item'); const l = is[is.length - 1]; if (l) { l.querySelectorAll('input')[0].value = e.degree || ''; l.querySelectorAll('input')[1].value = e.institution || ''; l.querySelectorAll('input')[2].value = e.year || ''; } });
...
function discardLinkedInData() { document.getElementById('linkedInPreview').style.display = 'none'; pendingLinkedInData = null; showToast('Discarded'); }
function parseLinkedInText(t) {
  ...
  if (es) { const el = es.split('\n').filter(l => l.trim()); for (let i = 0; i < Math.min(el.length, 20); i += 4) r.experiences.push({ title: el[i] || '', company: el[i + 1] || '', dates: el[i + 2] || '', description: el[i + 3] || '' }); }
  const ed = t.split(/education/i)[1];
  if (ed) { const dl = ed.split('\n').filter(l => l.trim()); for (let i = 0; i < Math.min(dl.length, 12); i += 3) r.education.push({ degree: dl[i] || '', institution: dl[i + 1] || '', year: dl[i + 2] || '' }); }
  const sm = t.match(/skills?[:\s]+([^\n]+)/i);
  if (sm) r.skills = sm[1];
  return r;
}
const dz = document.getElementById('linkedInDropZone');
if (dz) { dz.addEventListener('dragover', e => { e.preventDefault(); dz.style.borderColor = 'var(--primary)'; }); dz.addEventListener('dragleave', e => { e.preventDefault(); dz.style.borderColor = 'var(--border)'; }); dz.addEventListener('drop', e => { e.preventDefault(); dz.style.borderColor = 'var(--border)'; const f = e.dataTransfer.files[0]; if (f && f.type === 'application/pdf') { const dt = new DataTransfer(); dt.items.add(f); document.getElementById('linkedInFile').files = dt.files; importLinkedInPDF({ target: { files: dt.files } }); } }); }
function exportJSON() { const d = { version: '1.0', ...getState() }; saveAs(new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' }), 'cv-genie-backup.json'); }
function importJSON(e) { const f = e.target.files[0]; if (!f) return; const r = new FileReader(); r.onload = ev => { try { restoreState(JSON.parse(ev.target.result)); } catch (ex) { showToast('Invalid file'); } }; r.readAsText(f); }
function getAllText() { const parts = [document.getElementById('fullName')?.value || '', document.getElementById('title')?.value || '', document.getElementById('summary')?.value || '', document.getElementById('skills')?.value || '', ...getExperiences().map(e => [e.title, e.company, e.description].join(' ')), ...getEducation().map(e => [e.degree, e.institution].join(' '))]; return parts.join(' ').toLowerCase(); }
const ROLE_KW = {
...
function runATSAnalysis() {
  ...
  const matchedRole = Object.keys(ROLE_KW).filter(k => k !== 'default' && role.length >= 2 && (role.includes(k) || k.includes(role))).sort((a, b) => b.length - a.length)[0] || 'default';
  const roleKW = ROLE_KW[matchedRole];
  const allKW = [...roleKW, ...customKWList.filter(k => !roleKW.includes(k))];
  const results = allKW.map(kw => ({ keyword: kw, present: cvText.includes(kw.toLowerCase()), custom: customKWList.includes(kw) }));
  ...
  if (ring) { ring.style.strokeDasharray = C; ring.style.strokeDashoffset = C - (C * score / 100); ring.style.stroke = col; }
  ...
  if (ab) { ab.innerHTML = checks.map(b => `<div style="padding:0.25rem 0.3rem;border-radius:4px;font-size:0.65rem;display:flex;justify-content:space-between;align-items:center;${b.pass ? 'background:#d1fae5;color:#065f46' : 'background:#fee2e2;color:#991b1b'}"><span>${b.label}</span><span style="font-size:0.8rem;">${b.pass ? '✓' : '✗'}</span></div>`).join(''); }
  const kc = document.getElementById('keywordCloud');
  if (kc) { kc.innerHTML = results.map(r => { const isCustom = r.custom; const clickable = !r.present && !isCustom; const bg = r.present ? (isCustom ? 'background:#ede9fe;color:#5b21b6' : 'background:#d1fae5;color:#065f46') : (isCustom ? 'background:#f5f3ff;color:#7c3aed' : 'background:#fee2e2;color:#991b1b'); const cursor = clickable ? 'cursor:pointer;' : ''; const onclick = clickable ? `onclick="addKWFromCloud('${r.keyword.replace(/'/g, "\\'")}')"` : ''; const title = clickable ? `title="Click to add '${r.keyword}' to your custom keywords"` : ''; return `<span ${onclick} ${title} style="display:inline-flex;align-items:center;gap:0.15rem;padding:0.15rem 0.4rem;border-radius:8px;font-size:0.6rem;font-weight:600;${bg};${cursor}${clickable ? 'border:1.5px dashed currentColor;' : ''}">${r.keyword}${isCustom ? '<span style="font-size:0.5rem;margin-left:1px">★</span>' : ''}${clickable ? '<span style="font-size:0.55rem;opacity:0.7">+</span>' : ''}</span>`; }).join(''); }
  const C2 = 150.8;
  ...
  if (pc) { pc.innerHTML = checks.map(b => `<div class="ats-chk ${b.pass ? 'pass' : 'fail'}"><span>${b.label}</span><span>${b.pass ? '✓' : '✗'}</span></div>`).join(''); }
}
setInterval(() => { try { localStorage.setItem('cvGenie_v2.0', JSON.stringify(getState())); } catch (e) { } }, 300000);
function loadSaved() {
  const p = new URLSearchParams(location.search);
  if (p.get('load')) { try { restoreState(JSON.parse(LZString.decompressFromEncodedURIComponent(p.get('load')))); history.replaceState({}, document.title, location.pathname); return; } catch (e) { } }
  ...
  if (s) { try { restoreState(JSON.parse(s)); return; } catch (e) { } }
  addExpSilent();
  addEduSilent();
  buildAll();
  updatePreview();
...
document.addEventListener('keydown', e => { const tag = document.activeElement?.tagName; const isInput = tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT'; if ((e.ctrlKey || e.metaKey) && e.key === 's') { e.preventDefault(); } if ((e.ctrlKey || e.metaKey) && e.key === 'z' && !isInput) { e.preventDefault(); undo(); } if ((e.ctrlKey || e.metaKey) && e.key === 'y' && !isInput) { e.preventDefault(); redo(); } });
loadSaved();
if (window.innerWidth <= 900) { document.getElementById('formPanel').classList.add('active'); document.getElementById('mobileNav').classList.add('nav-hidden'); document.getElementById('mnav-edit').classList.add('active'); }